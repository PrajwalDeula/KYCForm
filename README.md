To run the project, the first thing that you need to do is open the Django project. Then, you use command "cd office" in your terminal and type "python manage.py runserver". By doing this, the web application runs. Do note that KYCForm is in its initial stages, that's why there are several types of issues. 

You can easily logins directly typing "http://127.0.0.1:8000/kyc_login/". Your page loads, where you can find two fields, particularly Policy Number and Date of birth for each individual policy holder with a unique identity. The front-end and back-end integration is not perfect due remaining development of Rest API that helps connection.
...
Once you provide the form with the mandatory details, the next thing you should focus on is viewing the form that comes in "http://127.0.0.1:8000/kyc_list/". The CRUD operation is utilised to offer you the update, view, and delete of the existing data. 
